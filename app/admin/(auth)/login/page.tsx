...
const AdminLoginPage: React.FC = () => {
  ...
  // Focus the first field the moment the page mounts, so a keyboard/screen
  // reader user doesn't need an extra Tab press just to reach the form.
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // TODO (integration): Replace this entire block with a real NextAuth
    // signIn('credentials', { username, password }) call. Never validate
    // credentials or set auth state on the client — anyone can open the
    // browser console and fake this cookie. Auth must be verified and the
    // session issued by the server.
    setTimeout(() => {
      ...
      // Placeholder only — remove once NextAuth is wired up.
  ...
  const toggleShowPassword = () => {
    ...
    // Keep focus on the password field so typing isn't interrupted.
    setTimeout(() => {
      document.getElementById("password")?.focus();
    }, 50);
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              <div>
                ...
                <input
                  ...
                  required
                  aria-required="true"
                  aria-invalid={statusMessage?.type === "error" && !username.trim()}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    aria-required="true"
                    aria-invalid={statusMessage?.type === "error" && !password.trim()}
                    aria-describedby={statusMessage ? "status-message" : undefined}
                    placeholder="Enter your password"
                    disabled={isLoading}
                    className="w-full px-4 py-3 border border-[#C5D8EC] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2B6CB0] focus:border-transparent transition bg-[#F8FAFE] text-[#1A1A1A] placeholder:text-[#5A6B7D] disabled:opacity-50 disabled:cursor-not-allowed pr-12"
                  />
                  <button
                    type="button"
